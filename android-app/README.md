...
- “数据与缓存”支持清理临时缓存、网站 Cookie、播放记录及恢复默认设置。
- 可导出脱敏诊断报告；报告不会包含 Cookie、查询参数或完整视频地址。

## 视频源管理

- `css1.json` 作为可选规则包下载到应用私有目录，解析时只读取本地副本。
- 可在“我的 → 视频源规则”下载、重新下载或卸载 CSS1 规则包；卸载后其中的视频源立即停用。
